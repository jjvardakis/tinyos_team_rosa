...
static int Scheduler_accesses=0;
/*
...
rlnode * sel;
  ...
  for(k=0;k<Priority_Queues;k++){
   sel = rlist_pop_front(& SCHED[k]);
  if(sel!=NULL){
    k=Priority_Queues;
  }
}
Scheduler_accesses++;  // for boost
...
//code for boost
   ...
   if(Scheduler_accesses>=100){
    Scheduler_accesses=0;
    for(int j=Priority_Queues-1;j>=1;j--){
      while( is_rlist_empty(&(SCHED[j]))!=0){
       
       rlist_push_back(& SCHED[0], rlist_pop_front(& SCHED[j]));
       fprintf(stderr, "Boost happened\n" );
      }
...
void gain(int preempt)
{
  ...
  bios_set_timer(QUANTUM+QUANTUM*(current->priority+1)/Priority_Queues);
}
